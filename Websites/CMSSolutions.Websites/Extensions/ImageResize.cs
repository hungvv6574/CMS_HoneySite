...
namespace CMSSolutions.Websites.Extensions
{
    ...
    {
        public Image ImageOutput {get; set; }
        public byte[] ImageBuffer { get; set; }
        public string ResizeImageThumbnail(string sourceFile, int maxWidth, int maxHeight)
        {
            return Resize(sourceFile, maxWidth, maxHeight);
        }

        public Image GetImage(string sourceFile)
        {
           return System.Drawing.Image.FromFile(HttpContext.Current.Server.MapPath(sourceFile));
        }

        public string Resize(string sourceFile, int maxWidth, int maxHeight, Image sourceImage = null, bool preserverAspectRatio = true, int quality = 100)
        {
            if (sourceImage == null)
            {
                sourceImage = GetImage(sourceFile);
            }
            ...
            if (extension == ".jpg" || extension == ".jpeg")
            {
                ...
                if (oCodec != null)
                {
                    ...
                    ImageBuffer = memo.GetBuffer();
                    memo.Close();
                    string base64String = Convert.ToBase64String(ImageBuffer);

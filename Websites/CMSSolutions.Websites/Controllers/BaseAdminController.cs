...
namespace CMSSolutions.Websites.Controllers
{
    public class BaseAdminController : BaseController
    {
        ...
        public IEnumerable<SelectListItem> BindCategoriesParent()
        {
            var languageCode = WorkContext.CurrentCulture;
        ...
        public virtual string BuildProductGroup()
        {
            var service = WorkContext.Resolve<IProductsGroupService>();
            var list = service.GetRecords();
            var sb = new StringBuilder();

            sb.AppendFormat(T("Dòng sản phẩm") + " <select id=\"" + Extensions.Constants.ProductGroup + "\" name=\"" + Extensions.Constants.ProductGroup + "\" autocomplete=\"off\" class=\"uniform form-control col-md-3\" onchange=\"$('#" + TableName + "').jqGrid().trigger('reloadGrid');\">");

            foreach (var obj in list)
            {
                sb.AppendFormat("<option value=\"{1}\">{0}</option>", obj.GroupName, obj.Id);
            }
        ...
        public virtual IEnumerable<SelectListItem> BindProductTypes()
        {
            var serviceType = WorkContext.Resolve<IProductVolumeService>();
            var list = serviceType.GetRecords();
            var result = new List<SelectListItem>();
        ...
        public virtual IEnumerable<SelectListItem> BindPaymentTypes()
        {
            var list = EnumExtensions.GetListItems<PaymentType>();
            var result = new List<SelectListItem>();
            foreach (var type in list)
            {
                var item = new SelectListItem();
        ...
        public virtual IEnumerable<SelectListItem> BindProductGroup()
        {
            var service = WorkContext.Resolve<IProductsGroupService>();
            var list = service.GetRecords();
            var result = new List<SelectListItem>();
        ...
        public virtual string BuildCategories(int defaultValue = 0, bool hasEvent = false)
        {
            var languageCode = WorkContext.CurrentCulture;
            ...
            if (hasEvent)
            {
                var url = Url.Action("GetArticlesByLanguage");
                sb.AppendFormat(T("Chuyên mục") + " <select id=\"" + Extensions.Constants.CategoryId + "\" name=\"" + Extensions.Constants.CategoryId + "\" autocomplete=\"off\" class=\"uniform form-control col-md-3\" onchange=\"" +
                " var data = $('#" + TableName + "_Container').find('select').serialize();" +
                    @"$.ajax({{
	                url: '" + url + @"',
	                data: data,
	                ...
	                dataType: 'json',
	                success: function (result) {{
                        $('#" + Extensions.Constants.ArticlesId + @"').empty();
                        if (result != null){{
		                    $.each(result, function(idx, item) {{                  
                                $('#" + Extensions.Constants.ArticlesId + @"').append($('<option>', {{
                                    value: item.Value,
                                    text: item.Text
                                }}));
                            }});   
